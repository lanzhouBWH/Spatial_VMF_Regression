Validation.Normal1<-function(fit,Data_list,iters,burnins){
  ...
  for(i in 1:I){
    PP_Matrix[,i,]<-sapply(1:V, function(v) rowMeans(pred_Matrix[,i,v,])) 
  }
  
  
  alpha=sapply(1:I, function(i) sapply(1:V, function(v) acos(PP_Matrix[,i,v]%*%Y[[i]][v,]/norm(PP_Matrix[,i,v],type = "2")) ))
  # S=diag(3)
